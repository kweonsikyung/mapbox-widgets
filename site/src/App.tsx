...
import RoutePlanningDemo from "./demos/RoutePlanningDemo";
import ZoneManagementDemo from "./demos/ZoneManagementDemo";
...
import {
  ...
  PreviewCompassRose,
  PreviewScaleBar,
  PreviewCoordinateDisplay,
  PreviewTimeSlider,
  PreviewMeasureTool,
  PreviewMeasureDisplay,
  PreviewInterpolate,
  PreviewHaversine,
...
const CATEGORIES: { label: string; color: string; items: CatItem[] }[] = [
  {
    ...
    items: [
      {
        name: "MapboxMap",
        desc: "Context provider — wrap all other components inside",
        Preview: PreviewMapboxMap,
      },
      {
        name: "GlobeToggle",
        desc: "Switch between globe and mercator projection",
        Preview: PreviewGlobeToggle,
      },
      {
        name: "SearchBox",
        desc: "Mapbox Geocoding autocomplete search",
        Preview: PreviewSearchBox,
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "RouteLayer",
        desc: "Render past (solid) and future (dashed) routes",
        Preview: PreviewRouteLayer,
      },
      {
        name: "MarkerLayer",
        desc: "Manage many custom DOM markers declaratively",
        Preview: PreviewMarkerLayer,
      },
      {
        name: "ShipMarker",
        desc: "Animated ship icon with optional pulse ring",
        Preview: PreviewShipMarker,
      },
      {
        name: "OverviewMap",
        desc: "Picture-in-picture minimap synced to main view",
        Preview: PreviewOverviewMap,
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "DrawLayer",
        desc: "Polygon, line, and point drawing on the map",
        Preview: PreviewDrawLayer,
      },
      {
        name: "DrawToolbar",
        desc: "Mode selector UI for DrawLayer",
        Preview: PreviewDrawToolbar,
      },
      {
        name: "RouteEditor",
        desc: "Draggable waypoint-based route builder",
        Preview: PreviewRouteEditor,
      },
      {
        name: "BBoxSelector",
        desc: "Click-drag to select a bounding box region",
        Preview: PreviewBBoxSelector,
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "ClusterLayer",
        desc: "Groups nearby points into expandable bubbles",
        Preview: PreviewClusterLayer,
      },
      {
        name: "HeatmapLayer",
        desc: "Density heatmap from GeoJSON point data",
        Preview: PreviewHeatmapLayer,
      },
      {
        name: "LayerPanel",
        desc: "Toggle visibility of any Mapbox layer",
        Preview: PreviewLayerPanel,
      },
      {
        name: "ContextMenu",
        desc: "Right-click context menu with custom actions",
        Preview: PreviewContextMenu,
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "ZoomControls",
        desc: "Zoom in / out buttons",
        Preview: PreviewZoomControls,
      },
      {
        name: "CompassRose",
        desc: "Bearing-aware compass needle",
        Preview: PreviewCompassRose,
      },
      {
        name: "ScaleBar",
        desc: "Distance scale in km, mi, or nautical miles",
        Preview: PreviewScaleBar,
      },
      {
        name: "CoordinateDisplay",
        desc: "Live cursor coordinates display",
        Preview: PreviewCoordinateDisplay,
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "TimeSlider",
        desc: "Scrub and animate a time range with playback",
        Preview: PreviewTimeSlider,
      },
      {
        name: "MeasureTool",
        desc: "Click-to-measure distance on the map",
        Preview: PreviewMeasureTool,
      },
      {
        name: "MeasureDisplay",
        desc: "Formatted readout for MeasureTool results",
        Preview: PreviewMeasureDisplay,
      },
      {
        name: "interpolatePosition",
        desc: "Util — interpolate lng/lat along a path at t∈[0,1]",
        Preview: PreviewInterpolate,
      },
      {
        name: "haversineDistance",
        desc: "Util — great-circle distance between two points",
        Preview: PreviewHaversine,
      },
    ],
  },
];
...
const FEATURES = [
  {
    Icon: Map,
    title: "Core Map",
    tags: ["MapboxMap", "GlobeToggle", "SearchBox"],
    desc: "MapboxMap context provider — drop any child component inside and it wires up automatically. GlobeToggle for 3D globe. SearchBox for Mapbox Geocoding.",
  },
  {
    Icon: Ship,
    title: "Route & Vessel",
    tags: ["RouteLayer", "MarkerLayer", "ShipMarker"],
    desc: "RouteLayer renders separate past (solid) and future (dashed) lines per route with hover tooltips and filtering. ShipMarker for animated vessel icons.",
  },
  {
    Icon: PenLine,
    title: "Drawing Tools",
    tags: ["DrawLayer", "RouteEditor", "BBoxSelector"],
    desc: "DrawLayer with undo/redo, select mode, and live vertex editing. RouteEditor with numbered waypoints, mid-segment insertion, and distance display. BBoxSelector for click-and-drag region queries.",
  },
  {
    Icon: BarChart3,
    title: "Data Layers",
    tags: ["ClusterLayer", "HeatmapLayer", "LayerPanel"],
    desc: "ClusterLayer groups nearby points into expandable bubbles. HeatmapLayer for density visualization. LayerPanel to toggle any Mapbox layer.",
  },
  {
    Icon: SlidersHorizontal,
    title: "Map Controls",
    tags: ["ZoomControls", "CompassRose", "ScaleBar"],
    desc: "Drop-in UI controls: ZoomControls (+/−), CompassRose (bearing-aware needle), ScaleBar (km / mi / NM), CoordinateDisplay, and ContextMenu.",
  },
  {
    Icon: Timer,
    title: "Timeline",
    tags: ["TimeSlider", "interpolatePosition"],
    desc: "TimeSlider for scrubbing and animating vessel playback at configurable speed. Wire onChange to interpolatePosition to drive marker positions.",
  },
  {
    Icon: Anchor,
    title: "Maritime Ops",
...
import "mapbox-gl/dist/mapbox-gl.css"`,
  ...
  usage: `import { MapboxMap, RouteLayer, ZoomControls, CompassRose } from "mapbox-gl-kit"
...
export function ShipMap() {
  ...
  draw: `import { useState, useRef } from "react"
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 14 }}>
        <Icon size={28} color={C.accent} strokeWidth={1.5} />
      </div>
      <div
        style={{
          fontSize: 16,
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 720 }}>
        ...
        >
          25 composable Mapbox GL JS components for React. Route visualization,
          drawing tools, data layers, and more.
        </p>
        ...
        >
          <span
            style={{ fontFamily: "var(--mono)", fontSize: 15, color: C.muted }}
          >
        ...
        >
          ...
          <a
            href="https://github.com/kweonsikyung/mapbox-widgets"
            target="_blank"
            rel="noopener"
            style={{
              padding: "13px 32px",
...
function Features() {
  return (
    <section style={{ padding: "80px 24px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      >
        Place any component inside <code>&lt;MapboxMap&gt;</code> and it wires
        up via React context — no prop drilling.
      </p>
      <div
        style={{
          display: "grid",
